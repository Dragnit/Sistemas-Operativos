    ...
    if(n > 0)
    {
        ...
        else 
        {
            ...
            if (n > 0) 
            {
                ...
                {
                    fprintf(mediana_file, "mediana: %.2f desvioPadrao: %.2f\n", mediana, desvioPadrao);                     
                    fclose(mediana_file);
                ...
                else 
                {
                    for (int k = 0; k < n; k++) 
                    {
                        fprintf(coluna_file, "%d ", valores[k]);
                    }
                    fclose(coluna_file);
                    printf("ficheiro ficha03_coluna.dat atualizado com sucesso!\n");
                }

                FILE *coluna_file2 = fopen("ficha03_coluna2.dat", "w");
                if (coluna_file == NULL) 
                {
                    printf("Erro ao abrir o arquivo ficha03_coluna.dat\n");
                } 
...
void imprimir_conteudo_ficheiro(FILE *file) 
{
    char linha[256];
...
void sigusr2_handler_filho2(int signum)
{
    ...
    FILE *file = fopen("ficha03_coluna.dat", "r");
    FILE *ficheiro2 = fopen("ficha03_mediana.dat", "r");

    if (file == NULL || ficheiro2 == NULL) {
        printf("Erro ao abrir o ficheiro ficha03_coluna.dat ou ficha03_mediana.dat\n");
        exit(1);
    }
    else
    {
        printf("Conteúdo do ficheiro ficha03_coluna.dat:\n");
        imprimir_conteudo_ficheiro(file);

        printf("\nConteúdo do ficheiro ficha03_mediana.dat:\n");
        imprimir_conteudo_ficheiro(ficheiro2);
    }
...
{
    const char *comando = "gnuplot -e \"set boxwidth 0.8; set style fill solid; plot 'ficha03_coluna2.dat' using 1 smooth frequency with boxes; pause mouse;\"";
        ...
        else
        {
            ...
            else
            {
                while(1)
                {
                    ...
                    {
                        remove("ficha03_mediana.dat");
                        remove("ficha03_coluna.dat");
...
void* transformaColuna(void* args) {
    char nomeFicheiro[25];
    char nomeFicheiro2[30];
    char linhas[1000];
    int numColuna;
    int numRepeticoes;

    char buffer[10000] = ""; // Buffer para armazenar os valores da coluna
    FILE* fp;
    FILE* invertido;

    printf("Introduza o nome do ficheiro sem a extensão do mesmo: ");
    scanf("%s", nomeFicheiro);
    ...
    if (invertido == NULL) {
        printf("Erro ao criar o ficheiro invertido!\n");
        fclose(fp);
        pthread_exit(NULL);
    }
    ...
    // Remover a última vírgula e adicionar uma nova linha
    if (strlen(buffer) > 0) {
        buffer[strlen(buffer) - 1] = '\n';
    }
    fprintf(invertido, "%s", buffer);
    fclose(fp);
    fclose(invertido);

    // Reabrir para leitura
    invertido = fopen(nomeFicheiro2, "r");
    if (invertido == NULL) 
    {
        printf("Erro ao abrir o ficheiro invertido para leitura!\n");
        pthread_exit(NULL);
    }

    char linha[1000];
    if (fgets(linha, sizeof(linha), invertido) == NULL) 
    {
        printf("Erro ao ler a linha do ficheiro %s\n", nomeFicheiro2);
        fclose(invertido);
        pthread_exit(NULL);
    }
    fclose(invertido);
    ...
    // Reabrir para append
    invertido = fopen(nomeFicheiro2, "a");
    if (invertido == NULL) 
    {
        printf("Erro ao abrir o ficheiro invertido para append!\n");
        pthread_exit(NULL);
    }

    for (int i = 0; i < numRepeticoes - 1; i++) {
        fprintf(invertido, "%s", linha);
    }
